    ...
    if ret:
        ...
        if len(faces) > 0:
            for (face_x, face_y, face_w, face_h) in faces:
                ...
                # Get the center coordinates of detected eyes
                for (eye_x, eye_y, eye_w, eye_h) in eyes:
                    eye_centers.append((face_x + int(eye_x + eye_w / 2), face_y + int(eye_y + eye_h / 2)))

                if len(eye_centers) >= 2:
                    # Calculate the size and position for the glasses
                    ...
                    # Determine the position of the glasses
                    ...
                    # Create an overlay image with the glasses
                    ...
                    # Extract background excluding glasses
                    background = cv2.bitwise_and(image, image, mask=mask)

                    # Create an inverse mask for the glasses
                    mask_inv = cv2.bitwise_not(mask)

                    # Extract glasses from the overlay
                    glasses = cv2.bitwise_and(overlay_image, overlay_image, mask=mask_inv)

                    # Combine the background with the glasses
